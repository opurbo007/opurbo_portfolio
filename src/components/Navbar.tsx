...
const Navbar = () => {
  ...
  return (
    <>
        ...
        <div className="flex items-center justify-end space-x-4">
          <Menubar>
            {menu.map((item, idx) => (
              <MenubarMenu key={idx}>
                <Link href={item.path}>
                  <MenubarTrigger className="text-lg">{item.title}</MenubarTrigger>
                </Link>
              </MenubarMenu>
            ))}
          </Menubar>
      ...
      {/* Mobile */}
      <div className="flex justify-between items-center top-0 left-0 w-full p-2 md:hidden">
        <div>
          <Link href={"/"}>
            ...
            <SheetHeader>
              <SheetTitle className="pt-6 flex items-center justify-center">
                <Link href={"/"} onClick={() => setOpen(false)}>
                  <Image src={"/opurbo.jpg"} alt={"logo"} width={200} height={100} />
                </Link>
              </SheetTitle>
              <SheetDescription>
                <Menubar className="flex flex-col border-none">
                  {menu.map((item, idx) => (
                    <MenubarMenu key={idx}>
                      <Link href={item.path} className="py-2" onClick={() => setOpen(false)}>
